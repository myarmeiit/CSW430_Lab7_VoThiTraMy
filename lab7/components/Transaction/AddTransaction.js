    ...
    try {
      const transaction = {
        ...
        services: selectedServices.map(service => ({
          // include several id variants to match backend expectations
          service: service._id,
        ...
        priceBeforePromotion: totalPrice,
        price: totalPrice,
      };

      // log payload for debugging
      console.log('Posting transaction payload:', JSON.stringify(transaction));
      ...
      try {
        ...
        console.warn('AsyncStorage unavailable, falling back to general.js token', e);
      }

      if (!tokenValue) {
        try {
          const {token: fallbackToken} = require('../../general/general');
          tokenValue = fallbackToken;
        } catch (e) {
          console.warn('No fallback token available', e);
        }
      }
      ...
      console.log('Create transaction response:', resp.data);
      navigation.goBack();
    } catch (error) {
      console.error('Error creating transaction:', error.response?.status, error.response?.data || error.message);
      const serverData = error.response?.data;
      const message = serverData?.message || JSON.stringify(serverData) || error.message;
      Alert.alert('Error creating transaction', message);
    }
  };
